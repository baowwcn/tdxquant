...
if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help"):
    print("用法:")
    print("  python query_etf_holdings.py <CODE>          查询持仓 (需通达信登录)")
    print("  python query_etf_holdings.py <CODE> --holder 查询持有人结构 (免登录)")
    print("示例:")
    print("  python query_etf_holdings.py 515180")
    print("  python query_etf_holdings.py 512170.SH --holder")
    sys.exit(0)

raw_code = sys.argv[1].replace(".SH", "").replace(".SZ", "")
FUND_ID = raw_code
ETF_CODE = f"{raw_code}.SH"
ETF_NAME = KNOWN_NAMES.get(raw_code, raw_code)

MODE = "holdings"
if len(sys.argv) > 2 and sys.argv[2] in ("--holder", "-r"):
    MODE = "holder"


def fetch_name_from_web():
    try:
        url = f"https://fund.eastmoney.com/pingzhongdata/{FUND_ID}.js"
...
def fetch_holdings():
    ...
    name = fetch_name_from_web()
    display_name = name or ETF_NAME

    print(f"{display_name} ({ETF_CODE}) 持仓查询\n{'-' * 60}")
    ...
    print(f"\n{'=' * 100}")
    print(f"{display_name} ({ETF_CODE}) 重仓成分股")
    print(f"{'=' * 100}")
    print(
...
def fetch_holder():
    name = fetch_name_from_web()
    display_name = name or ETF_NAME
    print(f"{display_name} ({ETF_CODE}) 持有人结构\n{'-' * 60}")
