...
def fetch_holdings():
    ...
    for i, h in enumerate(holdings):
        ...
        h["lc"] = snap.get("LastClose", "--") if snap else "--"
        if (i + 1) % 10 == 0:
    ...
    print(f"{'=' * 90}")
    print(
        f"{'#':<4} {'代码':<12} {'名称':<10} {'占净值%':<10} {'最新价':<10} {'前收盘':<10} {'总市值(亿)':<12} {'涨幅%':<8} {'市盈率':<8} {'换手%':<8}"
    )
    print("-" * 90)
    for i, h in enumerate(holdings):
        ...
        print(
            f"{i + 1:<4} {h['code']:<12} {h['name']:<9} {h['pct']:<10} {str(h['now']):<10} {str(h['lc']):<10} {zsz:<12} {zaf:<8} {pe:<8} {hsl:<8}"
        )

    total_pct = sum(float(h["pct"]) for h in holdings if h["pct"] not in ("--", None))
    print(f"\n持仓汇总:")
    print(f"  披露持仓数量: {len(holdings)} 只")
    print(f"  披露持仓占净值比例: {total_pct:.2f}%")

    today = datetime.date.today().strftime("%Y%m%d")
    csv_file = f"{ETF_CODE}_holdings_{today}.csv"
